...
graph TD;
    ...
    I --|Нет|--> K{C >= A && D <= B};
    K --|Да|--> L[Вывод: "C, D внутри A, B"];
    K --|Нет|--> M{C < A && D > B};
    M --|Да|--> N[Вывод: "C, D снаружи A, B"];
    M --|Нет|--> O{D <= B && C <= A};
    O --|Да|--> P[Вывод: "C, D слева и внутри A, B"];
    O --|Нет|--> Q{C >= A && D > B};
    Q --|Да|--> R[Вывод: "C, D справа и внутри A, B"];
    Q --|Нет|--> S[Вывод: "Пересечение"];
    H --> T([Конец]);
    J --> T;
    L --> T;
    N --> T;
